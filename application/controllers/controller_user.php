...
{ 
    ...
    function action_getPrivateRatingAndShow()
    {
          if (isset($_GET['user'])) {
              $userid = $_GET['user'];
              $rating = $this->model->getUserRating($userid);
          }
          $list = $this->model->getRating(); //вытаскиваем все строки из бд

        @$this->view->generate('privaterating_view.php', 'template_view.php', array('list' => $list, 'rating' => $rating));//передаем View
    }
