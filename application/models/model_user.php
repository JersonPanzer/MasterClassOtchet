...
class Model_User extends Model
{
    ...
    {
        $query = $this->DBH->prepare("SELECT info.family_name, users.user_id FROM info, users WHERE users.user_status='1' AND users.user_login=info.user_login");
        $query->execute();
    ...
    {
        $query = $this->DBH->prepare("SELECT event.name,staff.family_name,marks.mark,marks.visited FROM event,staff,marks,users WHERE users.user_id=:userid AND users.group_id=event.group_id AND users.user_id=marks.user_id AND staff.user_id=event.staff_id AND event.id=marks.event_id");
